...
```{r}
spotify_songs |> ggplot() + geom_histogram(aes(x=danceability), binwidth=0.05)
```
...
```{r}
spotify_songs |> ggplot() + geom_histogram(aes(x=danceability), binwidth=0.05) + geom_vline(aes(xintercept=mean(danceability)), color="red", linetype = "dashed", size=1)
```
...
This data is not normal; rather, it is slightly skewed to the left. The mean shows us that most songs have a high level of danceability, with the highest bin totals occurring over 0.50. But it's clear pretty much all songs have some danceability to them.
